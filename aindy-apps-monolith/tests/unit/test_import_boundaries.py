...
_AINDY_SPEC = importlib.util.find_spec("AINDY")
assert _AINDY_SPEC is not None and _AINDY_SPEC.submodule_search_locations
AINDY_ROOT = pathlib.Path(next(iter(_AINDY_SPEC.submodule_search_locations))).resolve()
...
def _iter_aindy_import_statements() -> list[tuple[str, str]]:
    ...
    for source_file in AINDY_ROOT.glob("**/*.py"):
        ...
        relative_path = source_file.relative_to(AINDY_ROOT.parent).as_posix()
