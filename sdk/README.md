...
result = client.memory.read("/memory/shawn/tasks/**", limit=20)
...
client.memory.read("/memory/shawn/tasks/*")         # one level
...
```python
client.events.emit("task.completed", {"task_id": "42", "duration_ms": 300})
```
