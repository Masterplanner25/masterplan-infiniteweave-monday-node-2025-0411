...
# Allow tests to run from the sdk/ directory without installing the package.
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

from aindy import AINDYClient
from aindy.exceptions import (
    AINDYError,
    AuthenticationError,
    NetworkError,
    NotFoundError,
...
class TestClientRequest(unittest.TestCase):
    ...
    def test_successful_post_returns_json(self):
        ...
        with patch("aindy.client.urlopen", return_value=mock_resp):
    ...
    def test_get_with_params_builds_query_string(self):
        ...
        with patch("aindy.client.urlopen", return_value=mock_resp) as mock_open:
    ...
    def test_network_error_raises_NetworkError(self):
        ...
        c = _make_client()
        with patch("aindy.client.urlopen", side_effect=URLError("Connection refused")):
    ...
    def test_http_401_raises_AuthenticationError(self):
        ...
        with patch("aindy.client.urlopen", side_effect=err):
...
class TestMemoryAPI(unittest.TestCase):
    ...
    def test_list_calls_memory_list(self):
        envelope = _ok_response({"nodes": []})
        with patch.object(self.client.syscalls, "call", return_value=envelope) as m:
            self.client.memory.list("/memory/shawn/tasks")
        m.assert_called_once_with(
            "sys.v1.memory.list",
            {"path": "/memory/shawn/tasks", "limit": 100},
        )
...
class TestFlowAPI(unittest.TestCase):
    ...
    def test_run_calls_flow_run_syscall(self):
        ...
        with patch.object(self.client.syscalls, "call", return_value=envelope) as m:
            result = self.client.flow.run("analyze_tasks", {"nodes": []})
        m.assert_called_once_with(
            "sys.v1.flow.run",
            {"flow_name": "analyze_tasks", "input": {"nodes": []}},
        )
...
class TestEventAPI(unittest.TestCase):
    ...
    def test_emit_calls_event_emit_syscall(self):
        ...
        with patch.object(self.client.syscalls, "call", return_value=envelope) as m:
            self.client.events.emit("task.completed", {"task_id": "42"})
        m.assert_called_once_with(
            "sys.v1.event.emit",
            {"type": "task.completed", "payload": {"task_id": "42"}},
        )
...
class TestIntegrationPipeline(unittest.TestCase):
    ...
    def test_read_flow_write_pipeline(self):
        client = _make_client()
        nodes = [{"id": "n1", "content": "Sprint goal"}]
        ...
        with patch.object(client.syscalls, "call", side_effect=call_results) as m:
            mem_result = client.memory.read("/memory/shawn/tasks/**")
            flow_result = client.flow.run("analyze_tasks", {"data": mem_result["data"]["nodes"]})
