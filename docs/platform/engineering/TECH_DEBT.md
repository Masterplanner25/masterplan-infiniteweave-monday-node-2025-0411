...
- **RESOLVED (Prompt 1):** The confirmed runtimeâ†’domain import violation in `AINDY/platform_layer/async_job_service.py` is gone. `_emit_job_log_written()` now emits the `job_log.written` registry event, and automation performs the sync from its own bootstrap subscriber. The runtime platform layer no longer imports this automation service directly. Primary files: `AINDY/platform_layer/async_job_service.py`, `apps/automation/bootstrap.py`.
- **RESOLVED (Prompt 2):** Automation syscall handlers no longer act as a cross-domain import bus. The audit snapshot that triggered Prompt 2 found direct imports into seven app domains plus duplicate task-handler registration. In the current workspace, `apps/automation/syscalls/syscall_handlers.py` routes cross-domain work through `get_dispatcher().dispatch(...)` wrappers and no longer re-registers task handlers. Primary files: `apps/automation/syscalls/syscall_handlers.py`, `apps/tasks/bootstrap.py`, `AINDY/kernel/syscall_dispatcher.py`.
- **RESOLVED (Prompt 9):** Stuck-run threshold inconsistency is fixed. `stuck_run_service.py` now reads `settings.STUCK_RUN_THRESHOLD_MINUTES`; the default is `45`; and `config.py` enforces `STUCK_RUN_THRESHOLD_MINUTES > FLOW_WAIT_TIMEOUT_MINUTES` with a model validator so legitimately waiting flows are not recovered as stuck. Primary files: `AINDY/config.py`, `AINDY/agents/stuck_run_service.py`.

- **LARGELY RESOLVED (2026-04-09):** Nodus/runtime execution is now converged end-to-end. Execution-record layer, runtime-result layer, and retry-policy layer were unified first. The above-runtime-boundary gap is now closed: `run_flow()`, `execute_intent()`, and `run_nodus_script_via_flow()` are syscall proxies that route through `SyscallDispatcher` → `PersistentFlowRunner`. All authenticated execution uses a single path. Remaining open gap: EU gate coverage in `goals_router.py`, `research_results_router.py`, `leadgen_router.py`, and `freelance_router.py` (tracked in execution-normalization item below). Primary files: `runtime/flow_engine.py`, `kernel/syscall_registry.py`.
- **LARGELY RESOLVED (2026-04-09):** Flow orchestration and VM execution are now converged through the syscall layer. `run_flow()` is a syscall proxy; WAIT/RETRY semantics and persistent flow lifecycle remain in `PersistentFlowRunner` (unchanged); retry is unified through `core/retry_policy.py`; route-level orchestration envelopes are auto-injected at `core/execution_pipeline.py`. Remaining open gap: same EU gate coverage as above. Primary files: `runtime/flow_engine.py`, `kernel/syscall_registry.py`.
- **PARTIAL:** Agentics enters the canonical Nodus/runtime path through `execute_agent_run_via_nodus()`. Per-step retry decisions now go through `resolve_retry_policy(execution_type="agent", risk_level=...)` — the hardcoded `1 if risk_level == "high" else MAX_STEP_RETRIES` branches are gone. Remaining debt: residual flow-shell orchestration above the canonical entrypoint in agent planning and lifecycle. Primary files: `runtime/nodus_adapter.py`, `runtime/flow_engine.py`, `agents/agent_runtime.py`.
- **PARTIAL:** Search orchestration is now materially unified. `apps/search/services/search_service.py` is the canonical durable, memory-aware search layer for SEO analysis, LeadGen preview, and Research query fallback, with shared cache/history reuse through `search_history`. The remaining gaps are narrower: LeadGen full generation still persists through the flow/syscall path, SEO meta generation is still route-local, and richer provider-backed parsing/ranking is still incomplete. Primary files: `apps/search/services/search_service.py`, `routes/seo_routes.py`, `routes/leadgen_router.py`, `routes/research_results_router.py`, `AINDY/db/models/search_history.py`, `routes/memory_router.py`.
- **PARTIAL:** Execution normalization is still incomplete at the system boundary. A canonical unification layer now exists in `core/execution_gate.py` — `require_execution_unit()` gates execution at route entry points, `to_envelope()` produces a shared `{eu_id, trace_id, status, output, error, duration_ms, attempt_count}` shape, and adapter functions map FlowRun/AgentRun/AutomationLog to that shape. Agent, automation, flow, and platform routes now embed `execution_envelope` in responses and create/attach ExecutionUnit before dispatch. Task, Genesis, ARM, Watcher, Goals, Research, LeadGen, and Freelance routes are now also covered in the verified router set. Remaining gap: other unverified route groups may still bypass the EU gate and should be audited separately. Primary remaining files: broader route surface audit pending beyond the verified set.

#### Medium

- **RESOLVED (Prompt 11):** `apps/_adapters.py` is no longer an unowned cross-cutting implementation module. The response adapters now live in `AINDY/platform_layer/response_adapters.py`, and `apps/_adapters.py` is only a backward-compatibility shim. Primary files: `AINDY/platform_layer/response_adapters.py`, `apps/_adapters.py`, `apps/tasks/bootstrap.py`, `apps/identity/bootstrap.py`.
- **RESOLVED (Prompt 12):** Duplicate task syscall registration is removed. `apps/tasks/bootstrap.py` is the canonical owner of task syscall registration, automation no longer re-registers those handlers, and `VersionedSyscallRegistry.__setitem__` now warns when a syscall name is re-registered with a different handler. Primary files: `AINDY/kernel/syscall_registry.py`, `apps/tasks/bootstrap.py`, `apps/automation/syscalls/syscall_handlers.py`.
- **RESOLVED (Prompt 13):** `apps/arm` no longer imports `apps/identity` directly for ARM analysis enrichment. Identity context and observation now cross the boundary through `sys.v1.identity.get_context` and `sys.v1.identity.observe`, and identity owns its own syscall registration path. Primary files: `apps/arm/services/deepseek/deepseek_code_analyzer.py`, `apps/identity/syscalls/syscall_handlers.py`, `apps/identity/bootstrap.py`.
- **RESOLVED (Prompt 6):** The default async thread pool is no longer sized for only four concurrent AI jobs. `AINDY_ASYNC_JOB_WORKERS` now defaults to `10`, `async_job_service.py` reads that setting from `config.py`, and startup logs warn when a thread-mode deployment is undersized for AI workloads. Primary files: `AINDY/config.py`, `AINDY/platform_layer/async_job_service.py`, `AINDY/main.py`.
- **RESOLVED (Prompt 14):** The client production build now fails fast on missing or localhost API configuration. `client/scripts/validate-env.js` validates `VITE_API_BASE_URL` before build, the package scripts enforce that check, production source maps are disabled, and platform-only UI code is split into a separate lazy-loaded chunk. Primary files: `client/vite.config.js`, `client/scripts/validate-env.js`, `client/package.json`.

- **PARTIAL:** RippleTrace now includes execution-causality structure on top of `SystemEvent`, including parent-child edges, event->memory links, and a proofboard-style viewer, but broader productization and deeper scenario coverage remain incomplete. Primary files: `core/system_event_service.py`, `apps/rippletrace/services/rippletrace_service.py`, `AINDY/db/models/ripple_edge.py`, `client/src/components/RippleTraceViewer.jsx`.
- **PARTIAL:** Freelancing remains incomplete. AI delivery generation, execution metrics, and external email/webhook delivery now exist, and Stripe payment-link mode is implemented for hosted checkout URL generation. Remaining debt: fulfillment webhook handling, payment-status confirmation, refunds, and subscription billing. Supported delivery types: `manual`, `email`, `webhook`, `payment`. Broader commercial workflow automation remains incomplete. Primary files: `apps/freelance/services/freelance_service.py`, `routes/freelance_router.py`, `apps/freelance/models/freelance.py`.
- **PARTIAL:** Infinity is now memory-weighted, system-state-aware, goal-aware, and expected-vs-actual-aware, but optimization depth remains shallow. Watcher, feedback, ranked memory signals, and prediction accuracy now feed decisions, yet KPI-weight learning and broader policy adaptation are still missing. Primary files: `apps/analytics/services/infinity_service.py`, `apps/analytics/services/infinity_loop.py`, `memory/memory_scoring_service.py`, `platform_layer/system_state_service.py`, `routes/main_router.py`.
- **RESOLVED (2026-04-23):** ARM analyzer config is DB-backed and cross-instance. ConfigManager.get_all() re-reads the DB on every call; DeepSeekCodeAnalyzer calls _refresh_runtime_config(db) at the start of every run_analysis() and generate_code() invocation. A SystemEvent (`arm.config.updated`) is emitted on `PUT /arm/config`. Tests in `tests/unit/test_arm_config_propagation.py` prove cross-instance propagation. Primary files: `apps/arm/services/deepseek/config_manager_deepseek.py`, `apps/arm/routes/arm_router.py`.
...
- **PARTIAL:** Live route-level learning behavior is now additive but still incomplete. Shared search flows now expose a stable `learning_context` block in Research, LeadGen preview/cached responses, and SEO analysis, and the legacy Infinity compute route now surfaces `memory_influence`; however, other route groups and some non-preview LeadGen paths still do not expose recalled memory or learning influence consistently. Primary files: `apps/search/services/search_service.py`, `routes/research_results_router.py`, `routes/leadgen_router.py`, `routes/seo_routes.py`, `routes/main_router.py`, `memory/memory_scoring_service.py`, `apps/analytics/services/infinity_orchestrator.py`.

#### Low

- **PARTIAL:** Cache backend is configurable for Redis and startup now enforces cache/Redis coherence, but multi-instance correctness still depends on explicit Redis configuration and availability in deployment. Primary files: `config.py`, `main.py`, `routes/health_router.py`.
...
- Shared orchestration helpers in `runtime/nodus_execution_service.py` now also back scheduled Nodus execution, more of platform execution, and the canonical agent entrypoint, so older notes that describe those orchestration surfaces as fully separate are stale.
- Shared durable search orchestration now lives in apps/search/services/search_service.py, so older notes that describe SEO, LeadGen preview, and Research fallback as fully separate route-local search implementations are stale.
- Shared route-level search learning context now flows from `apps/search/services/search_service.py` into Research, LeadGen preview/cached responses, and SEO analysis, so older notes that describe those route payloads as having no visible recalled-memory context are stale.
- Execution normalization (`execution_envelope`) now covers task_router, genesis_router, arm_router, watcher_router, goals_router, research_results_router, leadgen_router, and freelance_router in the verified router set.
- §12.3 (async embedding) is resolved — `memory/embedding_jobs.py` with `enqueue_embedding()` and `EMBEDDING_RETRY_DELAYS` replaces the synchronous OpenAI call; older open-status note in §12.3 is stale.
- §15.15 (Execution Loop Console) is now RESOLVED: `FlowEngineConsole.jsx` wired into `App.jsx` at `/flow-console` (2026-04-09).
- Syscall convergence (2026-04-09): the "above the runtime boundary" execution unification gap is now largely resolved. `run_flow()`, `execute_intent()`, and `run_nodus_script_via_flow()` are syscall proxies routing through `SyscallDispatcher`; four new syscalls registered (`sys.v1.flow.execute_intent`, `sys.v1.nodus.execute`, `sys.v1.job.submit`, `sys.v1.agent.execute`). See `docs/architecture/SYSCALL_SYSTEM.md §10`.
...
- Search System is now materially more unified across SEO, LeadGen, and Research through `apps/search/services/search_service.py`, with durable history/cache reuse in `search_history` and consistent memory-aware context for shared search flows. Remaining gaps: LeadGen full generation still splits between the durable shared layer and the flow/syscall persistence path, SEO meta generation is not part of durable search history, and richer provider-backed parsing/ranking is still missing. Canonical reference: `docs/apps/SEARCH_SYSTEM.md`.
- Freelancing System lacks automation and AI generation; metrics are incomplete. Canonical reference: `docs/apps/FREELANCING_SYSTEM.md`.
- ? **RESOLVED (2026-03-21):** Social Layer visibility scoring + bridge event persistence implemented; social posts now log to Memory Bridge with DB session. Canonical reference: `docs/apps/SOCIAL_LAYER.md`.
- RippleTrace is **partially implemented** beyond signal capture; pattern services, graph logic, and a frontend viewer now exist, but execution-causality tracing and the full insight layer remain incomplete. Canonical reference: `docs/apps/RIPPLETRACE.md`.
- Masterplan SaaS no longer lacks anchor/ETA support; the remaining debt is dependency cascade modeling and execution automation on top of the existing planning + activation layer. Canonical reference: `docs/apps/MASTERPLAN_SAAS.md`.
...
- **RESOLVED (2026-04-23):** `AINDY/bridge/archive/` deleted. `memory_bridge_core_draft.rs` and `Memorybridgerecognitiontrace.rs` removed.

## 2. Schema / Migration Debt
- Migration drift risk exists due to multiple overlapping migrations and no automated migration validation in deployment (`alembic/versions/`).
...
- ? **RESOLVED (2026-03-22):** Legacy rows in `tasks`, `leadgen_results`, and `authors` were checked with `tools/maintenance/backfill_user_ids.py` (dry-run) and no NULL `user_id` rows remain.
...
- No automated migration validation tests (`alembic/` has no test harness).
...
- ? **RESOLVED (current workspace):** Silent `except ...: pass` blocks removed from production code under `routes/`, `services/`, `runtime/`, `db/`, `apps/`, and `watcher/`. Replacement behavior is structured logging, observability events, or explicit propagation depending on callsite criticality.
...
- ? **RESOLVED (current workspace):** Lease timestamps in `apps/tasks/services/task_service.py` now use timezone-aware UTC and normalize loaded DB values before comparison. The worker startup warning `can't compare offset-naive and offset-aware datetimes` is eliminated in live compose startup.
...
- Shared helpers in `runtime/nodus_execution_service.py` now also normalize the Nodus execution summary/record consumed by `nodus_execute_node()`, direct memory execution, and `routes/platform_router.py`, reducing record-shape drift at the runtime boundary.
...
- Infinity Algorithm Support System no longer operates as a score-only open loop. Watcher, explicit feedback capture, and `services/infinity_loop.py` are implemented. Remaining gaps are expanded TWR weighting, ranking, and learned optimization. Canonical reference: `docs/apps/INFINITY_ALGORITHM_SUPPORT_SYSTEM.md`.
...
- **`memory_links.strength` is defined as `VARCHAR(20)` with default `"medium"`.** This means relationship weight is a non-comparable string enum (`"low"`, `"medium"`, `"high"`). It cannot be used in ORDER BY relevance, cannot be averaged, and cannot participate in any scoring formula. Any future graph traversal that needs weighted edges will require a schema migration to convert this to a numeric type.
  - Location: `alembic/versions/bff24d352475_create_memory_nodes_links.py`, `AINDY/services/memory_persistence.py` (MemoryLinkModel)
...
- **`memory_nodes` has an `updated_at` column but no history table, no append-only log, and no event sourcing.** When a node's content is updated, the prior value is permanently overwritten. The stated design objective of reconstructing past states across sessions cannot be fulfilled without a record of mutations.
  - Location: `AINDY/services/memory_persistence.py` (MemoryNodeModel  no history table), `alembic/versions/` (no history migration)
...
- **`MemoryNodeDAO.save()` calls OpenAI synchronously** before the DB insert. A slow or failed OpenAI API call delays the HTTP response. Failure falls back to zero vector (safe), but latency is not bounded.
  ...
  - Status: ? **RESOLVED (current workspace):** Embedding generation is now fully async. `memory/embedding_jobs.py` implements `enqueue_embedding()` dispatched through `core/execution_dispatcher.py` with `EMBEDDING_RETRY_DELAYS = (1, 2, 4)`. Write path stores nodes immediately with `embedding_status="pending"` and backfills via async job. `async_job_service.py` registers the job handler via `memory/embedding_jobs` late-bound import. Primary files: `memory/embedding_jobs.py`, `db/dao/memory_node_dao.py`, `platform_layer/async_job_service.py`.
...
- Canonical metrics unique constraint migration: `alembic/versions/97ef6237e153_structure_integrity_check.py:24`
...
- ? **RESOLVED (2026-03-22 Quick Wins):** `test_migrations.py` rewritten. Root cause: local `alembic/` directory has `__init__.py` which shadows the installed alembic package when using `python -m alembic`. Fix: use `shutil.which("alembic")` to find the console-script entry point and call it directly. Test skips gracefully if DB is unavailable (test environment) rather than failing.
...
- **`apps/arm/services/deepseek/__pycache__/` contains both cpython-311 and cpython-314 compiled bytecode.** This indicates the project has been executed under Python 3.14 (pre-release as of 2026-03). Mixed pycache creates confusion about which Python version is authoritative and may cause subtle import resolution issues.
  - Location: `apps/arm/services/deepseek/__pycache__/`
...
- ? **RESOLVED (2026-04-09):** `FlowEngineConsole.jsx` is now imported and routed at `/flow-console` in `App.jsx`. The component surfaces FlowRuns, resume, registry, automation logs, and scheduler status. `ExecutionConsole.jsx` remains at `/console` as the TWR/calculation console.
  - Location: `client/src/components/FlowEngineConsole.jsx`, `client/src/App.jsx`
...
- ? **RESOLVED (current workspace):** `RippleTraceViewer.jsx` now exists and is routed from `client/src/App.jsx`. The viewer renders a signal timeline and graph-based ripple surface against the active RippleTrace APIs.
  - Location: `client/src/components/RippleTraceViewer.jsx`, `client/src/App.jsx`
...
- ? **RESOLVED (current workspace):** A dedicated approval surface now exists in `client/src/components/AgentApprovalInbox.jsx`, routed at `/agent/approvals` from `client/src/App.jsx`.
- ? **RESOLVED (current workspace):** Sidebar-level pending approval visibility now exists in `client/src/components/Sidebar.jsx` via the approval count badge and `APPROVAL_EVENT` refresh hook.
...
- There is now one canonical Nodus execution substrate, but orchestration semantics still live in a separate layer:
  - `runtime/flow_engine.py` for persistent flow orchestration, WAIT/RESUME, and strategy-driven routing
  - `runtime/nodus_execution_service.py` plus `runtime/nodus_runtime_adapter.py` for canonical VM/source execution
- This split is narrower than before because Nodus result summaries now share one builder, but it still creates naming confusion and higher-level envelope drift between orchestration runs and direct runtime execution.
  - Primary files: `AINDY/runtime/flow_engine.py`, `AINDY/runtime/nodus_adapter.py`, `AINDY/runtime/nodus_execution_service.py`, `AINDY/runtime/nodus_runtime_adapter.py`, `AINDY/routes/platform_router.py`, `AINDY/routes/memory_router.py`, `AINDY/platform_layer/async_job_service.py`
  - Status: Partial. Structural debt.

### Section 16.8 [AGENTICS] Infinity loop is integrated but not autonomous - Open
- **The Infinity loop is now closed and memory-weighted, but the decision engine is still shallow and post-hoc.** `apps/analytics/services/infinity_loop.py` now consumes ranked memory signals in addition to KPI and feedback context, and persists `LoopAdjustment` / `UserFeedback`, but it does not yet:
...
- ? **RESOLVED (current workspace):** Compatibility routes now exist for `/analyze_seo/`, `/generate_meta/`, and `/suggest_improvements/`, and shared durable search orchestration now lives in `apps/search/services/search_service.py`.
  - Location: `client/src/api.js`, `AINDY/routes/seo_routes.py`, `apps/search/services/search_service.py`
...
- **Historical audit entries now contain stale open items that are already resolved in the workspace** (for example MasterPlan anchor/ETA, RippleTrace viewer, Observability dashboard, and normalized identity evolution summary). This document therefore needs periodic alignment to remain trustworthy as an audit artifact.
  - Location: `docs/platform/engineering/TECH_DEBT.md`
...
- ? **RESOLVED (current workspace):** Dedicated scenario coverage now proves that a high-impact failure can be emitted as a `SystemEvent`, linked through RippleTrace and memory capture, and change the next Infinity decision on a later run.
  - Location: `AINDY/tests/system/test_memory_driven_infinity_decisions.py`, `AINDY/core/system_event_service.py`, `apps/rippletrace/services/rippletrace_service.py`, `AINDY/memory/memory_scoring_service.py`, `apps/analytics/services/infinity_orchestrator.py`, `apps/analytics/services/infinity_loop.py`
