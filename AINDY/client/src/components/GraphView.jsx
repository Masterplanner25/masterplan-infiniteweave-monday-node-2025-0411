...
import { getCausalGraph, getInfluenceGraph, getNarrative } from "../api";
...
export default function GraphView() {
  ...
  useEffect(() => {
    ...
    const nodes = graphs.influence.nodes.map((node) => ({ ...node }));
    const rawEdges =
    ...
    const maxScore = Math.max(...nodes.map((node) => node.narrative_score || 0), 1);
          ...
          <>
            <section>
              ...
              <ul style={{ margin: "6px 0 0", paddingLeft: 16, fontSize: 12 }}>
                {narrative.timeline?.slice(-6).map((event, idx) => (
            ...
            <section>
              ...
              <ul style={{ margin: "6px 0 0", paddingLeft: 16, fontSize: 12 }}>
                {narrative.inflection_points?.map((point, idx) => (
