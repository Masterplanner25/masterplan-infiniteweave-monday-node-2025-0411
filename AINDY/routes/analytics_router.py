...
async def ingest_linkedin_manual(
    ...
    return await execute_with_pipeline(
        request=request,
        route_name="analytics.linkedin.manual",
        handler=handler,
...
async def get_masterplan_analytics(
    ...
    def handler(ctx):
        ...
        return query.order_by(CanonicalMetricDB.period_start.desc()).all()

    return await execute_with_pipeline(
        request=request,
        route_name="analytics.masterplan.get",
        handler=handler,
        user_id=str(current_user["sub"]),
        metadata={"db": db},
    )
...
async def get_masterplan_summary(
    ...
    return await execute_with_pipeline(
        request=request,
        route_name="analytics.masterplan.summary",
        handler=handler,
        user_id=str(current_user["sub"]),
        metadata={"db": db},
    )
