"""
services/api_key_service.py â€” Platform API key management.

Key generation and hashing
---------------------------
  generate_key()     â€” returns (raw_key, key_hash) â€” raw key is NEVER stored.
  hash_key(raw_key)  â€” SHA-256 hex digest used as the DB lookup token.

Key lifecycle
--------------
  create_api_key()   â€” inserts a new PlatformAPIKey record; returns (record, raw_key).
                       The raw key is returned exactly once and never retrievable again.
  revoke_api_key()   â€” sets revoked_at to now(); ownership-checked.
  list_api_keys()    â€” returns safe public metadata (no hash, no plaintext).
  touch_last_used()  â€” non-blocking last_used_at stamp called from auth path.
...
_KEY_PREFIX = "aindy_"
_KEY_TOKEN_BYTES = 32  # 32 bytes â†’ 43-char url-safe base64 (without padding)
...
def generate_key() -> tuple[str, str]:
    ...
    raw_key  â€” the plaintext key to deliver to the caller (aindy_<token>).
    key_hash â€” SHA-256 hex digest to store in the database.
    """
