...
- **PARTIAL:** Nodus/runtime execution is now converged at the execution-record layer, runtime-result layer, and retry-policy layer. Shared helpers in `runtime/nodus_execution_service.py` and `core/execution_record_service.py` normalize Nodus execution summaries and `execution_record` blocks. Retry decisions are now unified through `core/retry_policy.py` across flow_engine, nodus_adapter, async_job_service, and nodus_schedule_service. The remaining debt is above the runtime boundary: `PersistentFlowRunner` and route-level orchestration surfaces still do not form one end-to-end execution model. Primary files: `runtime/flow_engine.py`, `routes/platform_router.py`, `routes/memory_router.py`, `agents/agent_runtime.py`.
- **PARTIAL:** Flow orchestration and VM execution still drift semantically above the now-unified retry and record layers. WAIT/RETRY semantics and persistent flow lifecycle are centralized in `PersistentFlowRunner`; retry decisions are now unified through `core/retry_policy.py` (including per-run node overrides via `flow["node_configs"]`); but route-level orchestration envelopes still live partly above it. Primary files: `runtime/flow_engine.py`, `routes/platform_router.py`, `platform_layer/async_job_service.py`.
