...
- **OPEN:** Remaining cross-user exposure on 5 routes — `GET /leadgen/`, `GET /freelance/orders`, `GET /freelance/feedback`, `GET /research/`, `GET /rippletrace/pings`, `GET /rippletrace/drop_points`. These models lack `user_id` columns. Each requires a `user_id` column migration and service-layer filter before user scoping is possible. `leadgen_results`, `freelance_orders`, `client_feedback`, `research_results`, and rippletrace tables are missing `user_id`.
