...
- Ownership tables with `user_id` stored as `String` (e.g., `research_results`, `freelance_orders`, `client_feedback`, `drop_points`, `pings`) cannot have FK constraints to `users.id` until columns are normalized to UUID. This blocks strict ownership enforcement at the DB layer.
