"""
event_service.py — External webhook subscriptions for SystemEvents.
...
Subscription matching:
  "execution.completed"   — exact match
  "execution.*"           — prefix wildcard (any event starting with "execution.")
  "*"                     — global wildcard (every event)

Delivery:
  - Dispatched in a background daemon thread — never blocks the event path.
  ...
  - All delivery failures are logged and swallowed — a failing webhook
...
# subscription_id → metadata dict
_SUBSCRIPTIONS: dict[str, dict[str, Any]] = {}

# Bounded pool — prevents thread explosion under bursts of events
...
def _matches(pattern: str, event_type: str) -> bool:
    ...
    "*"            — matches everything
    "execution.*"  — matches any event whose type starts with "execution."
    exact string   — case-sensitive equality
    """
    if pattern == "*":
        return True
    if pattern.endswith(".*"):
...
def _load_subscription(
    ...
    *,
    ...
    Idempotent — skips silently if the id is already loaded.
    """
    with _webhook_lock:
...
def dispatch_webhooks(
    *,
    ...
    Intended to be called from a background thread — never call from the
...
def dispatch_webhooks_async(
    *,
    ...
    Returns immediately — caller is never blocked by delivery.
    """
    ...
    try:
        ...
        # Executor was shut down (e.g. during test teardown) — log and skip
