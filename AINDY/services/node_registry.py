"""
node_registry.py — Runtime external node registration.

Supports two node types:

  webhook   — The node POSTs state to an external HTTP endpoint and expects
  ...
  plugin    — Dynamically imports a Python function from the restricted
...
def _make_webhook_node(
    ...
    Request body (JSON):
        {
            ...
            "state":     {...}   # full node state — no DB objects
        }
    ...
    def webhook_node(state: dict, context: dict) -> dict:
        try:
            ...
            # Build a safe, serialisable payload — strip everything that
