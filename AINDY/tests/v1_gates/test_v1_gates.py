...
Test-to-task mapping:
    ...
    V1-VAL-014  test_domain_routes_gated_by_flag           V1-REFACT-013
...
def test_domain_routes_gated_by_flag(client):
    """
    V1-VAL-014 | Task: V1-REFACT-013
    When ENABLE_DOMAIN_APPS is False (default), domain-specific routes
    must not appear in the OpenAPI spec.
    This test verifies the platform surface is clean for V1 by default.
    """
    enable_domain = os.environ.get("ENABLE_DOMAIN_APPS", "false").lower()
    if enable_domain in ("1", "true", "yes"):
        pytest.skip("ENABLE_DOMAIN_APPS=true — domain routes are intentionally mounted")
    ...
    assert not exposed_domain_routes, (
        f"Domain routes are exposed in the default OpenAPI spec "
        f"({len(exposed_domain_routes)} routes):\n"
