...
def test_task_create_emits_task_created_event(client, auth_headers, db_session, test_user):
    ...
    """
    before = _count_events_of_type(db_session, SystemEventTypes.TASK_CREATED)
    ...
    events = _events_of_type(db_session, SystemEventTypes.TASK_CREATED)
    assert len(events) > before, (
        f"Expected at least one new '{SystemEventTypes.TASK_CREATED}' event "
    ...
    assert "name" in payload_keys, (
        f"No new '{SystemEventTypes.TASK_CREATED}' event has 'name' in payload. "
        f"Payloads: {[ev.payload for ev in new_events]}"
    )
    assert "task_id" in payload_keys, (
        f"No new '{SystemEventTypes.TASK_CREATED}' event has 'task_id' in payload. "
        f"Payloads: {[ev.payload for ev in new_events]}"
    )
...
def test_task_start_emits_task_started_event(db_session, test_user):
    ...
    before = _count_events_of_type(db_session, SystemEventTypes.TASK_STARTED)

    start_task(db_session, name, user_id=_TEST_USER_ID)

    after = _count_events_of_type(db_session, SystemEventTypes.TASK_STARTED)
    assert after > before, (
        f"Expected at least one new '{SystemEventTypes.TASK_STARTED}' event "
...
def test_task_complete_emits_task_completed_event(db_session, test_user):
    ...
    before = _count_events_of_type(db_session, SystemEventTypes.TASK_COMPLETED)

    complete_task(db_session, name, user_id=_TEST_USER_ID)

    after = _count_events_of_type(db_session, SystemEventTypes.TASK_COMPLETED)
    assert after > before, (
        f"Expected at least one new '{SystemEventTypes.TASK_COMPLETED}' event "
        f"(before={before}, after={after})"
    )
...
def test_task_pause_emits_task_paused_event(db_session, test_user):
    ...
    before = _count_events_of_type(db_session, SystemEventTypes.TASK_PAUSED)

    pause_task(db_session, name, user_id=_TEST_USER_ID)

    after = _count_events_of_type(db_session, SystemEventTypes.TASK_PAUSED)
    assert after > before, (
        f"Expected at least one new '{SystemEventTypes.TASK_PAUSED}' event "
        f"(before={before}, after={after})"
    )
...
def test_genesis_message_emits_started_event(client, auth_headers, db_session, test_user):
    ...
    """
    before = _count_events_of_type(db_session, SystemEventTypes.GENESIS_MESSAGE_STARTED)
    ...
    after = _count_events_of_type(db_session, SystemEventTypes.GENESIS_MESSAGE_STARTED)
    assert after > before, (
        f"Expected at least one new '{SystemEventTypes.GENESIS_MESSAGE_STARTED}' event "
