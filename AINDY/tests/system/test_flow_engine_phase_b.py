...
class TestFlowEngineCore:
    ...
    def test_register_node_decorator(self):
        from services.flow_engine import NODE_REGISTRY, register_node
    ...
    def test_resolve_next_node_simple(self):
        from services.flow_engine import resolve_next_node
    ...
    def test_enforce_policy_blocks_node(self):
        from services.flow_engine import POLICY, enforce_policy
    ...
    def test_execute_node_unknown_raises(self, mock_db):
        from services.flow_engine import execute_node
    ...
    def test_compile_plan_to_flow(self):
        from services.flow_engine import compile_plan_to_flow
...
class TestRuntimeRedirects:
    ...
    def test_execution_loop_redirects_to_flow_engine(self):
        """runtime/execution_loop.py re-exports from flow_engine."""
        from runtime import execution_loop

        assert hasattr(execution_loop, "PersistentFlowRunner")

    def test_execution_loop_preserves_execution_loop_class(self):
        """ExecutionLoop class is still accessible (existing code depends on it)."""
        from runtime import execution_loop

        assert hasattr(execution_loop, "ExecutionLoop")
...
class TestGeneratePlanFromIntent:
    ...
    def test_arm_analysis_plan(self):
        from services.flow_engine import generate_plan_from_intent
