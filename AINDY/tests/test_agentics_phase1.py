...
class TestAgentRouterEndpoints:
    ...
    def test_create_run_requires_auth(self, client):
        resp = client.post("/agent/run", json={"goal": "test"})
    ...
    def test_get_run_requires_auth(self, client):
        resp = client.get("/agent/runs/fake-id")
        assert resp.status_code in (401, 403)

    def test_approve_requires_auth(self, client):
        resp = client.post("/agent/runs/fake-id/approve")
        assert resp.status_code in (401, 403)

    def test_reject_requires_auth(self, client):
        resp = client.post("/agent/runs/fake-id/reject")
        assert resp.status_code in (401, 403)

    def test_steps_requires_auth(self, client):
        resp = client.get("/agent/runs/fake-id/steps")
        assert resp.status_code in (401, 403)
    ...
    def test_trust_get_requires_auth(self, client):
        resp = client.get("/agent/trust")
        assert resp.status_code in (401, 403)

    def test_trust_put_requires_auth(self, client):
        resp = client.put("/agent/trust", json={"auto_execute_low": True})
        assert resp.status_code in (401, 403)

    def test_trust_get_authenticated(self, client, auth_headers, mock_db):
        # mock_db uses chained MagicMock — first() returns None (no trust settings)
        resp = client.get("/agent/trust", headers=auth_headers)
        assert resp.status_code == 200
        data = resp.json()
        assert "auto_execute_low" in data
        assert "auto_execute_medium" in data
        assert "note" in data

    def test_trust_put_authenticated(self, client, auth_headers, mock_db):
        mock_trust = MagicMock(
            auto_execute_low=True,
            auto_execute_medium=False,
        )
        mock_trust.updated_at = None
        mock_db.first.return_value = mock_trust
        resp = client.put(
            "/agent/trust",
            headers=auth_headers,
            json={"auto_execute_low": True},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert "auto_execute_low" in data

    def test_list_runs_authenticated(self, client, auth_headers, mock_db):
        mock_db.all.return_value = []
        resp = client.get("/agent/runs", headers=auth_headers)
        assert resp.status_code == 200
        assert isinstance(resp.json(), list)

    def test_tools_list_authenticated(self, client, auth_headers):
    ...
    def test_create_run_empty_goal_returns_400(self, client, auth_headers, mock_db):
        resp = client.post("/agent/run", json={"goal": ""}, headers=auth_headers)
