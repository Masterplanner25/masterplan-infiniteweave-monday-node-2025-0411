...
class TestParseSyscallName:
    from services.syscall_versioning import parse_syscall_name
...
class TestValidatePayload:
    def test_empty_schema_always_valid(self):
        from services.syscall_versioning import validate_payload
    ...
    def test_validate_input_alias(self):
        from services.syscall_versioning import validate_input
...
class TestValidateOutput:
    def test_valid_output(self):
        from services.syscall_versioning import validate_output
...
class TestVersionedSyscallRegistry:
    def _make_entry(self, cap="test.cap"):
        from services.syscall_registry import SyscallEntry
        return SyscallEntry(handler=lambda p, c: {}, capability=cap)

    def test_flat_set_and_get(self):
        from services.syscall_registry import VersionedSyscallRegistry
...
class TestDispatcherInputValidation:
    def setup_method(self):
        ...
        from services.syscall_registry import SYSCALL_REGISTRY, SyscallEntry
...
class TestDispatcherVersionFallback:
    ...
    def test_fallback_enabled_routes_to_latest_stable(self):
        ...
        try:
            ...
            import importlib
            import services.syscall_versioning as _ver_mod
            # Patch resolve_version to use fallback=True
            with patch.object(_ver_mod, "SYSCALL_VERSION_FALLBACK", True):
                with patch("services.syscall_dispatcher.SYSCALL_VERSION_FALLBACK", True):
                    result = self.dispatcher.dispatch(
    ...
    def test_resolve_version_no_fallback(self):
        from services.syscall_versioning import resolve_version
