def test_post_ai_productivity_boost(client, auth_headers):
    ...
    response = client.post("/ai_productivity_boost", json=payload, headers=auth_headers)
...
def test_post_income_efficiency(client, auth_headers):
    ...
    response = client.post("/income_efficiency", json=payload, headers=auth_headers)
    assert response.status_code == 200
    assert "Income Efficiency" in response.json()

def test_get_results_initial(client, auth_headers):
...
def test_post_execution_speed(client, auth_headers):
    ...
    response = client.post("/execution_speed", json=payload, headers=auth_headers)
    assert response.status_code == 200
...
def test_post_engagement_rate(client, auth_headers):
    ...
    response = client.post("/engagement_rate", json=payload, headers=auth_headers)
    assert response.status_code == 200
...
def test_post_lost_potential(client, auth_headers):
    ...
    response = client.post("/lost_potential", json=payload, headers=auth_headers)
    assert response.status_code == 200
...
def test_post_decision_efficiency(client, auth_headers):
    ...
    response = client.post("/decision_efficiency", json=payload, headers=auth_headers)
    assert response.status_code == 200
    assert "Decision Efficiency" in response.json()

def test_get_results_after_efficiency(client, auth_headers):
    response = client.get("/results", headers=auth_headers)
    assert response.status_code == 200
    assert isinstance(response.json(), list)
...
def test_post_batch_calculations(client, auth_headers):
    ...
    response = client.post("/batch_calculations", json=payload, headers=auth_headers)
    assert response.status_code == 200
    assert isinstance(response.json(), dict)
    assert "AI Productivity Boost" in response.json()

def test_get_results_after_batch(client, auth_headers):
    response = client.get("/results", headers=auth_headers)
    assert response.status_code == 200
    assert isinstance(response.json(), list)   
