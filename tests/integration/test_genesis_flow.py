...
def _read_source(relative_path: str) -> str:
    ...
    route_redirects = {
        "routes/genesis_router.py": Path(__file__).resolve().parents[2]
        / "apps"
        / "masterplan"
        / "routes"
        / "genesis_router.py",
        "routes/main_router.py": Path(__file__).resolve().parents[2]
        / "apps"
        / "analytics"
        / "routes"
        / "main_router.py",
        "domain/masterplan_service.py": Path(__file__).resolve().parents[2]
        / "apps"
        / "masterplan"
        / "services"
    ...
    full = route_redirects.get(relative_path, Path(__file__).resolve().parents[1] / relative_path)
...
class TestValidateDraftIntegrity:
    ...
    def test_audit_system_prompt_exists(self):
        from apps.masterplan.services.genesis_ai import AUDIT_SYSTEM_PROMPT
    ...
    def test_validate_draft_integrity_has_failsafe(self):
        ...
        from apps.masterplan.services.genesis_ai import validate_draft_integrity
        with patch("apps.masterplan.services.genesis_ai.perform_external_call", side_effect=Exception("Network error")):
    ...
    def test_validate_draft_integrity_failsafe_audit_passed_false(self):
        ...
        from apps.masterplan.services.genesis_ai import validate_draft_integrity
        with patch("apps.masterplan.services.genesis_ai.perform_external_call", side_effect=Exception("fail")):
            result = validate_draft_integrity(SAMPLE_DRAFT)
    ...
    def test_validate_draft_integrity_happy_path(self):
        ...
        with patch("apps.masterplan.services.genesis_ai.perform_external_call", return_value=mock_response):
            result = validate_draft_integrity(SAMPLE_DRAFT)
    ...
    def test_genesis_ai_exports_validate_draft_integrity(self):
        """validate_draft_integrity must be importable from apps.masterplan.services.genesis_ai."""
        try:
            from apps.masterplan.services.genesis_ai import validate_draft_integrity
        except ImportError:
            pytest.fail("validate_draft_integrity not importable from apps.masterplan.services.genesis_ai")
...
class TestMasterplanFactoryHardening:
    ...
    def test_factory_returns_masterplan_object(self, db_session, test_user):
        ...
        with patch("memory.memory_capture_engine.MemoryCaptureEngine.evaluate_and_capture"), patch(
            "apps.identity.services.identity_service.IdentityService.observe"
        ):
...
class TestSynthesisPromptSchema:
    ...
    def test_synthesis_prompt_schema_complete(self):
        from apps.masterplan.services.genesis_ai import SYNTHESIS_SYSTEM_PROMPT
        required = [
...
class TestPostureDescriptionHelper:
    """Tests for posture_description() from apps.masterplan.services.posture."""
