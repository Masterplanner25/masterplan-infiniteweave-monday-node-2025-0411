...
class TestMemoryRouterEndpoints:
    ...
    def test_create_link_value_error_returns_422(self, client, auth_headers, mock_db):
        ...
        # The route now checks node existence via get_by_id before calling
        # create_link.  Stub both lookups to return a mock node so the test
        # reaches the ValueError → HTTP 422 path.
        mock_node = MagicMock()
        app.dependency_overrides[get_db] = lambda: mock_db
        with patch.object(MemoryNodeDAO, "get_by_id", return_value=mock_node), \
             patch.object(MemoryNodeDAO, "create_link", side_effect=ValueError("does not exist")):
            resp = client.post(
                "/memory/links",
