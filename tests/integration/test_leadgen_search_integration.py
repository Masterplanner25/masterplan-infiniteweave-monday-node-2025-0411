from apps.search.services.leadgen_service import run_ai_search, _extract_leads_from_text, _extract_leads_from_response
...
def test_run_ai_search_uses_external_search(monkeypatch):
    ...
    monkeypatch.setattr("apps.search.services.research_engine.web_search", _fake_search)
...
def test_run_ai_search_falls_back_on_error(monkeypatch):
    ...
    monkeypatch.setattr("apps.search.services.research_engine.web_search", _boom)
    results = run_ai_search("test query")
    assert results
...
def test_search_scoring_for_seo():
    from apps.search.services.search_scoring import score_seo_result
