...
class TestInfinityLoopHelpers:
    ...
    def test_normalize_trigger_event_task_completion(self):
        from apps.analytics.services.infinity_loop import _normalize_trigger_event
    ...
    def test_serialize_adjustment_none(self):
        from apps.analytics.services.infinity_loop import serialize_adjustment
...
class TestTaskReprioritization:
    ...
    def test_reprioritize_tasks_updates_top_tasks_to_high(self):
        from apps.analytics.services.infinity_loop import _reprioritize_tasks
...
class TestRunLoop:
    ...
    def test_run_loop_persists_adjustment_with_next_action(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "apps.analytics.services.infinity_loop.get_latest_adjustment",
    ...
    def test_run_loop_never_returns_empty_next_action(self, monkeypatch):
        ...
        db.refresh.return_value = None
        monkeypatch.setattr(
            "apps.analytics.services.infinity_loop._decide",
...
class TestScoreRouterLoopSurface:
    ...
    def test_get_score_includes_latest_adjustment_when_score_exists(
        ...
        with patch("apps.masterplan.routes.score_router._latest_adjustment_payload", return_value={
    ...
    def test_recalculate_calls_orchestrator(self, client, auth_headers, mock_db, mocker):
        ...
        mocker.patch("apps.masterplan.routes.score_router._latest_adjustment_payload", return_value=None)
...
class TestLoopTriggerWiring:
    ...
    def test_agent_runtime_calls_orchestrator(self):
        src = pathlib.Path("apps/agent/agents/runtime_extensions.py").read_text(encoding="utf-8")
        assert "infinity_orchestrator" in src
...
class TestInfinityOrchestrator:
    ...
    def test_execute_returns_score_adjustment_and_next_action(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "apps.analytics.services.infinity_orchestrator.calculate_infinity_score",
        ...
        monkeypatch.setattr(
            "apps.analytics.services.infinity_orchestrator.run_loop",
            lambda user_id, trigger_event, db, score_snapshot=None: fake_adjustment,
        )
        monkeypatch.setattr(
            "apps.analytics.services.infinity_orchestrator.serialize_adjustment",
            lambda adjustment: {
                "id": "adj-1",
                "decision_type": "continue_highest_priority_task",
                "applied_at": "2026-01-01T00:00:00+00:00",
