...
class TestBuildKpiContextBlock:
    ...
    def test_returns_empty_when_no_snapshot(self):
        ...
        mock_db = MagicMock()
        with patch("apps.analytics.services.infinity_service.get_user_kpi_snapshot", return_value=None):
    ...
    def test_returns_block_when_snapshot_exists(self):
        ...
        mock_db = MagicMock()
        snap = self._snapshot()
        with patch("apps.analytics.services.infinity_service.get_user_kpi_snapshot", return_value=snap):
            result = _build_kpi_context_block("user-123", mock_db)
    ...
    def test_never_raises_on_import_error(self):
        ...
        with patch("AINDY.agents.agent_runtime._build_kpi_context_block", wraps=_build_kpi_context_block):
            with patch("apps.analytics.services.infinity_service.get_user_kpi_snapshot", side_effect=RuntimeError("fail")):
                result = _build_kpi_context_block("user-123", mock_db)
        assert result == ""
