...
def test_agent_run_rejects_token_for_missing_user(client):
    missing_user_id = uuid.uuid4()
    headers = {
        "Authorization": f"Bearer {create_access_token({'sub': str(missing_user_id), 'email': 'missing-agent@aindy.test'})}"
    }

    response = client.post("/apps/agent/run", json={"goal": "test"}, headers=headers)
...
def test_agent_run_get_rejects_cross_user_run(
    client,
    db_session,
):
    owner_id = uuid.UUID("00000000-0000-0000-0000-000000000013")
    caller_id = uuid.UUID("00000000-0000-0000-0000-000000000014")
    db_session.add_all(
        [
            User(
                id=owner_id,
                email="agent-owner@aindy.test",
    ...
    run = _seed_run(
        ...
        status="pending_approval",
        trace_id="trace-agent-hidden",
    )
    caller_headers = {
        "Authorization": f"Bearer {create_access_token({'sub': str(caller_id), 'email': 'agent-caller@aindy.test'})}"
    }

    response = client.get(f"/apps/agent/runs/{run.id}", headers=caller_headers)
