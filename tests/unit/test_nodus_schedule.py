...
Patch paths (all lazy imports — must patch at source module):
  ...
  is_background_leader → apps.tasks.services.task_service.is_background_leader
...
import apps.automation.models       # noqa: F401
...
class TestRunScheduledNodusJob:
    ...
    def test_skips_when_not_leader(self):
        ...
        with patch("apps.tasks.services.task_service.is_background_leader", return_value=False), \
    ...
    def test_skips_when_job_not_found(self):
        ...
        with patch("apps.tasks.services.task_service.is_background_leader", return_value=True), \
    ...
    def _run_with_result(self, flow_result, job=None):
        ...
        with patch("apps.tasks.services.task_service.is_background_leader", return_value=True), \
             patch("AINDY.db.database.SessionLocal", return_value=mock_db), \
             patch("AINDY.db.models.nodus_scheduled_job.NodusScheduledJob"), \
             patch("apps.automation.models.AutomationLog", return_value=mock_log), \
    ...
    def test_runner_exception_sets_error_status_does_not_raise(self):
        ...
        with patch("apps.tasks.services.task_service.is_background_leader", return_value=True), \
             patch("AINDY.db.database.SessionLocal", return_value=mock_db), \
             patch("AINDY.db.models.nodus_scheduled_job.NodusScheduledJob"), \
             patch("apps.automation.models.AutomationLog", return_value=MagicMock()), \
    ...
    def test_leader_check_exception_skips_gracefully(self):
        ...
        with patch("apps.tasks.services.task_service.is_background_leader", side_effect=RuntimeError("no db")), \
             patch("AINDY.db.database.SessionLocal") as mock_session:
