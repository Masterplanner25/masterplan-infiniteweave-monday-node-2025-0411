...
def test_masterplan_activate_node_success(monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.masterplan.services.masterplan_execution_service.sync_masterplan_tasks",
        lambda **kwargs: {"generated": 2, "skipped": False},
    )
    monkeypatch.setattr(
        "apps.masterplan.services.masterplan_execution_service.get_masterplan_execution_status",
...
def test_observability_scheduler_status_node_success(monkeypatch):
    ...
    monkeypatch.setattr("apps.tasks.services.task_service.is_background_leader", lambda: True)
...
def test_misc_extended_flow_nodes_smoke(monkeypatch):
    import apps.arm.services.arm_metrics_service as arm_metrics_module
    import apps.masterplan.services.goal_service as goal_service
    import apps.search.services.search_service as search_service
    import apps.arm.services.deepseek.config_manager_deepseek as config_module
