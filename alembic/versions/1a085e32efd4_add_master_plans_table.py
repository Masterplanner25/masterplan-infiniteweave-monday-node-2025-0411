...
def upgrade() -> None:
    ...
    # On a fresh DB the legacy metrics tables and memory_* tables already exist
    # (created by earlier migrations 94bcd9284285 and c7602451aabb). Skip them
    # if present so this migration is idempotent on new installs.
    conn = op.get_bind()
    ...
    for tbl, idx, cols in _legacy_metrics:
        if tbl not in existing:
            op.create_table(tbl, *cols)
            op.create_index(op.f(idx), tbl, ['id'], unique=False)

    # master_plans is the primary purpose of this migration
    if 'master_plans' not in existing:
    ...
    # memory_nodes and memory_links already exist from c7602451aabb on fresh DBs
    if 'memory_nodes' not in existing:
    ...
    if 'memory_links' not in existing:
